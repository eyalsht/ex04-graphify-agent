...
1. Open or create the matching test file in `tests/<module>/`, where `<module>` ∈
   `{extractor, graph_reader, vault, brief, gatekeeper, token_comparison, sdk, cli}`.
2. Write a test that **describes the desired behavior**, not the implementation.
3. Use descriptive test names: `test_unseeded_ranking_falls_back_to_centrality`, not `test_rank`.
4. Cover the **specific edge case** you're targeting in this cycle.
5. Run the test — it MUST fail:
   ```bash
   uv run pytest tests/vault/test_ranking.py::test_unseeded_ranking_falls_back_to_centrality -xvs
   ```
6. Verify the failure message is meaningful (e.g., `AttributeError: module 'repo_atlas.vault.ranking' has no attribute 'rank_nodes'`), not a syntax error or import error in unrelated code.

**Commit RED:**
```bash
git add tests/
git commit -m "test(vault): failing test for unseeded hot ranking"
```
...
```bash
git add src/
git commit -m "feat(vault): make seed_id optional in rank_nodes()"
```
...
5. **Coverage is a side effect, not the goal.** ≥ 90% coverage (CLAUDE.md §2) emerges naturally from disciplined TDD. Don't write tests just to inflate coverage on a function that has no logic.

## What "minimal" means in GREEN

Pseudo-example drawn from `docs/PRD.md` R2.1. The graph comes from the fixture factory —
never from a committed real artifact (CLAUDE.md §7). Test:
```python
def test_degree_counts_both_endpoints_of_every_edge():
    g = build_graph(graph_factory(nodes=["a", "b", "c"], links=[("a", "b"), ("a", "c")]))
    assert degree(g)["a"] == 2
```
...
- Touch multiple `src/repo_atlas/` modules
- Verify the `sdk.py` façade surface end-to-end
- Confirm the full pipeline (extractor → graph_reader → vault → brief) works

Use the per-module test directories for everything else. All tests run **keyless** — with no
key set the gatekeeper falls back to its offline mock client (CLAUDE.md §3), so no provider
