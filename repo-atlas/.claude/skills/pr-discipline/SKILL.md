...
This runs the entire quality stack — repo-atlas's actual gates (CLAUDE.md §2/§3):
- `uv run ruff check .` (0 violations required)
- `uv run mypy --strict src/` (0 errors)
- `uv run pytest --cov=src --cov-fail-under=90` (keyless — no provider API key set)
- file-size check — every Python file (tests too) ≤150 lines
- no-hardcoded-values check — config from JSON/env only, secrets via `os.environ`, and no
  target-repo node id or source path as a literal anywhere in `src/`
...
- New generated artifacts (`graph.json`, a vault, `BRIEF.md`) → describe them, never commit them
...
- [ ] No accidental file inclusions (e.g., `.DS_Store`, IDE configs, a target repo you mapped)
- [ ] No generated atlas output committed (`.atlas/`, vaults, briefs)
...
One paragraph: what does this PR do, in user-facing terms? Not "added function X" but "`hot.md` now ranks by centrality alone when no `--seed` is given, instead of silently scoring every node zero."

### Linked planning items

Reference the PRD section, requirement IDs, TODO items, and any related issues:
```
- Implements PRD.md R3.3 (seed-free hot ranking)
- Closes TODO items: PHASE3-005, PHASE3-006
- Related issue: #7
```
...
```
- Added src/repo_atlas/vault/ranking.py with optional seed proximity weighting
- Added tests/vault/test_ranking.py with 12 keyless tests
